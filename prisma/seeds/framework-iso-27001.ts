...
// Implementation guidance (formerly the separate ISO 27002 framework). ISO 27002
// is the implementation guidance for the same Annex A controls, so it lives here
// on the ISO 27001 controls rather than as a duplicate framework.
...
export async function seedIso27001(prisma: PrismaClient) {
  ...
  for (const domain of ISO_27001_DOMAINS) {
    ...
    for (const control of controls) {
      const guidance = GUIDANCE_ADDITIONS[control.code] ?? null;
      await prisma.control.upsert({
        where: { domainId_code: { domainId: createdDomain.id, code: control.code } },
        update: { name: control.name, description: control.description, guidance, order: control.order },
        create: { ...control, guidance, domainId: createdDomain.id },
      });
    }
  }
